    ...
    return (
      <button
        ...
        style={{
          ...
          color: '#fca5a5', fontSize: '0.6rem', padding: 0,
        }}
      >
...
const ChatWindow = () => {
  ...
  const [atBottom, setAtBottom]   = useState(true);  // pastda turibmizmi
  const [newCount, setNewCount]   = useState(0);     // yuqorida ekanda kelgan yangi xabarlar
  ...
  // Skroll holatini kuzatish — pastga yaqinmizmi (80px tolerans)
  const handleScroll = () => {
  ...
  // Yangi xabar kelganda: pastda bo'lsak — avto skroll; yuqorida bo'lsak — hisoblagich
  useEffect(() => {
    if (minimized) return;
  ...
  // Oldingi xabarlar yuklanayotganda skroll pozitsiyasini saqlash
  useEffect(() => {
    if (loadingMore) {
  ...
  return (
    <>
      <style>{`
        ...
        .chat-send-btn:hover:not(:disabled) { filter: brightness(1.1); transform: scale(1.05); }
      ...
      <div style={{
        ...
        maxHeight: minimized ? 60 : 'min(520px, calc(100dvh - 150px))',
        borderRadius: '1.25rem',
        boxShadow: '0 16px 56px rgba(99,102,241,0.2), 0 2px 12px rgba(0,0,0,0.08)',
        ...
        >
          ...
          <div style={{ position: 'relative' }}>
            ...
            <span style={{
              ...
              background: isOtherOnline ? '#34d399' : '#9ca3af',
              border: '2px solid #6366F1', transition: 'background 0.4s',
            }} />
          </div>
          ...
          <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
            <button onClick={() => setMinimized(p => !p)} style={headerBtnStyle} title={minimized ? 'Kattalashtirish' : 'Kichraytirish'}>
              <FiChevronDown size={14} style={{ transform: minimized ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }} />
            </button>
            <button onClick={closeConversation} style={headerBtnStyle} title="Yopish">
              <FiX size={14} />
            </button>
          </div>
        </div>
          ...
          <>
            ...
            >
                ...
                <div style={{ display: 'flex', justifyContent: 'center', padding: '6px 0 4px' }}>
                  <button
                    ...
                    style={{
                      ...
                      background: 'var(--bg-card, #fff)',
                      border: '1px solid var(--border, #e5e7eb)',
                      borderRadius: '2rem', padding: '5px 14px',
              ...
              {loadingMessages ? (
                ...
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', flex:1, gap:8, color:'var(--text-muted)', padding:'32px 20px', textAlign:'center' }}>
                  <FiMessageCircle size={38} style={{ opacity:0.2 }} />
                  <div style={{ fontWeight:700, fontSize:'0.88rem' }}>Suhbatni boshlang!</div>
                  <div style={{ fontSize:'0.75rem', opacity:0.7, maxWidth:200, lineHeight:1.5 }}>Mehmonxona egasiga savol yuboring</div>
                </div>
              ) : (
                grouped.map(item => {
                  if (item.type === 'date') {
                    return (
                      <div key={item.key} style={{
                        ...
                        margin:'10px 0 6px', fontSize:'0.66rem',
                  ...
                  return (
                    <div key={item.key} style={{
                      ...
                      alignItems:'flex-end', gap:6, marginBottom:1,
                      animation: 'fadeUp 0.18s ease',
                      opacity: isPending ? 0.75 : isFailed ? 0.85 : 1,
                    }}>
                      {!isMine && <Avatar user={msg.sender} size={24} />}
                      <div style={{ maxWidth: '75%' }}>
                        {/* Sender nomi — boshqalarning xabarlarida */}
                        {!isMine && msg.sender?.name && (
                          <div style={{
                            fontSize: '0.65rem', fontWeight: 700,
                            color: '#6366F1', marginBottom: 2, paddingLeft: 4,
                          }}>
                            {msg.sender.name}
                          </div>
                        )}
                      <div style={{
                        ...
                        <div>{msg.content}</div>
                        <div style={{
                          fontSize:'0.6rem', opacity:0.65, marginTop:3,
                          ...
                          {isMine && (
                            <>
                              {/* O'qildi / yuborildi / pending / failed */}
                              {isFailed ? (
            ...
            {/* Pastga tushish / yangi xabar tugmasi */}
            {!atBottom && (
              <button
            ...
            <form onSubmit={handleSend} style={{
              ...
              background:'var(--bg-card, #fff)', flexShrink:0, alignItems:'flex-end',
            }}>
              <textarea
                ref={inputRef}
                value={input}
                ...
                style={{
                  ...
                  borderRadius:'0.875rem', padding:'9px 12px',
                  fontSize:'0.875rem', fontFamily:'inherit',
                  background:'var(--bg-hover, #f9fafb)',
                  color:'var(--text-main)', outline:'none',
                  lineHeight:1.45, maxHeight:88, overflowY:'auto',
              ...
              <button
                ...
                style={{
                  ...
                  display:'flex', alignItems:'center', justifyContent:'center',
                  flexShrink:0, transition:'background 0.18s, transform 0.1s, filter 0.15s',
                }}
              >
                <FiSend size={15} />
              </button>
